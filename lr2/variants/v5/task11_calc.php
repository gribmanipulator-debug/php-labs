    ...
    <form method="post" action="task11_result.php" class="demo-form">
        <div class="form-row">
            <div>
                ...
                <input type="number" id="x" name="x" step="any" value="<?= htmlspecialchars($_GET['x'] ?? '1') ?>" placeholder="Введіть X" required>
            </div>
            <div>
                <label for="y">Значення Y</label>
                <input type="number" id="y" name="y" step="any" value="<?= htmlspecialchars($_GET['y'] ?? '5') ?>" placeholder="Введіть Y" required>
            </div>
        </div>
